		 			...
		 			<br />
		 				<div align="left" class="form-group">
							...
							<sec:authorize access="hasRole('ADMINISTRATOR') or hasRole('SUPER_ADMIN')">
								<spring:url value="/students/edit/${student.id}/" var="editUrl" />
								<spring:url value="javascript:void(0);" var="uploadUrl" />
								<spring:url value="/students/delete/${student.id}/" var="deleteUrl" />
								<a href="${editUrl}"><button class="btn btn-magnolia" type="submit" formmethod="get" formaction="${editUrl}">
								<div class="lsf-icon" title="setting"><spring:message code="students.showStudent.button.edit" /></div></button></a> |
								<a href="${uploadUrl}" id="uploadImage"><button class="btn btn-magnolia_two" type="submit" formaction="${uploadUrl}">
								<div class="lsf-icon" title="upload"><spring:message code="students.showStudent.button.upload" /></div></button></a> |		
								<a href="${deleteUrl}"><button class="btn btn-danger" type="submit" formmethod="get" formaction="${deleteUrl}">
								<div class="lsf-icon" title="trash"><spring:message code="students.showStudent.button.delete" /></div></button></a>		
							</sec:authorize>
							
							<div id="imageAttachmentUpload" hidden="true">
								<h3><spring:message code="students.showStudent.label.upload" /></h3>
								<form:form action="fileupload" modelAttribute="uploadItem" method="POST" name="fileuploadForm" enctype="multipart/form-data">
									...
									<input name="uploadType" type="hidden" value="student" />
									<label for="file"><spring:message code="students.showStudent.label.file" /></label>
									<input name="file" type="file" /><br />
									<button class="btn btn-magnolia_two" type="submit" formmethod="post" formaction="fileupload">
										<div class="lsf-icon " title="upload"><spring:message code="students.showStudent.button.upload" /></div>
									</button>					
								</form:form>
							</div>
		 			</div>
