...
@Entity
@Table(name="WEB_RSSSON_STUDENT_COURSE_RESOURCES")
@NamedQueries({
	@NamedQuery(name="StudentCourseResource.findById", query="select distinct s from StudentCourseResource s where s.id=:id"),
	@NamedQuery(name="StudentCourseResource.findAll", query="select s from StudentCourseResource s")
})
public class StudentCourseResource implements Serializable {
	...
	@Id
	@Column(name="STUDENT_COURSE_RESOURCE_ID")
